...
<body>
<form class ="seminarform" action ="seminarupdate.com">

	<c:forEach var="item" items="${datas}" >
	<input type="hidden" name ="seq" value =${item.seq }>
	<label class ="startdaylabel" >StartDay </label><input type ="date" name="startday" value=${item.startday }>
	<label class ="enddaylabel">EndDay </label><input type ="date" name="endday" value=${item.endday }>
	<input type="submit" value="수정하기" class ="btn">
	<br>
	<label class ="titlelabel">Title </label><input type="text" name="title" size="40" value =${item.title }>
	<br>
	<label class ="locationlabel">지역</label>
	<input type="text" size="60" name="location" value =${item.location }>
	
	<br>
	<textarea cols="200" rows="20" name="content">${item.content }</textarea>
	<br>
	<label class ="hashtaglabel">hashTags( , 로 구분)</label>
	<br>
	
	<textarea cols="200" rows="3" name="hashtag" >${item.hashtag }</textarea>
	</c:forEach>
